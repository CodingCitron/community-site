...
const PostPage = () => {
    ...
    const handleSubmit = async (event: FormEvent) => {
        ...
        try {
            const res = await axios.post(`/posts/${post?.identifier}/${post?.slug}/comments`, {
                body: newComment
            })
    ...
    const vote = async (value: number, comment?: Comment) => {
        // 로그인 상태가 아니라면 login 페이지 이동
        if(!authenticated) router.push("/login") // 나중에 로그인 할건지 물어보는 것으로 수정
        ...
        try {
            await axios.post('/votes', {
                identifier, slug, commentIdentifier: comment?.identifier,
                value
            })
                            ...
                            {comments?.map(comment => (
                                <div className='flex' key={comment.identifier}>
                                    ...
                                    <div className='flex-shrink-0 w-10 py-2 text-center rounded'>
                                        ...
                                        <p className='text0x font-bold'>{comment.voteScore}</p>
                                        {/* 싫어요 */}
                                        <div 
                                            className='w-6 mx-auto text-gray-400 rounded cursor-pointer hover:bg-gray-300 hover:text-blue-500'
                                            onClick={() => vote(-1, comment)}
